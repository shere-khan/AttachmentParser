...
def process(xml_file):
    ...
    # Create Key Contact SQL
    if '<Key_Contacts:Key_Contacts' in open(xml_file).read():
        create_key_contacts_sql(xml_file, root, ns, gov_tracking_no)
    
    # Extract Assurances Form XML string
    if '<SF424B:Assurances' in open(xml_file).read():
        create_assurances_sql(xml_file, root, ns, gov_tracking_no)

    # Extract Human Subject Form XML String
    # if '<ProtectionofHumanSubjects:ProtectionofHumanSubjects' in open(xml_file).read():
        # create_human_subjects_sql(xml_file, root, ns, gov_tracking_no)

    # Extract Disc Lobby Form XML String
    if '<SFLLL_1_2:LobbyingActivitiesDisclosure' in open(xml_file).read():
        create_disc_lobby_sql(xml_file, root, ns, gov_tracking_no)

def create_human_subjects_sql(xml_file, root, ns, gov_tracking_no):
...
def create_key_contacts_sql(xml_file, root, ns, gov_tracking_no):
...
def create_disc_lobby_sql(xml_file, root, ns, gov_tracking_no):
